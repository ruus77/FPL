...
conv1d_model = Conv1DRegressor(
    n_features=N_FEATURES_SEQ, seq_len=hipers.seq_len, channels=[32, 16], kernel_size=3, dropout=0.2
).to(device)
...
MODEL_REGISTRY = {
    mlp.__class__.__name__: (mlp, train_dl_mlp, valid_dl_mlp, value_idx_mlp, "mlp", data.shape[1], target_scaler),
    conv1d_model.__class__.__name__: (conv1d_model, train_dl_seq, valid_dl_seq, value_idx_seq, "conv1d", data_seq.shape[1], target_scaler_seq),
    lstm_model.__class__.__name__: (lstm_model, train_dl_seq, valid_dl_seq, value_idx_seq, "lstm", data_seq.shape[1], target_scaler_seq)
}
...
for model_name, (model, tr_dl, va_dl, v_idx, m_type, num_features, scaler) in MODEL_REGISTRY.items():
    ...
    optimizer = torch.optim.AdamW(model.parameters(), lr=hipers.lr, weight_decay=hipers.weight_decay)
...
for model_name, (model, _, _, v_idx, m_type, _, _) in MODEL_REGISTRY.items():
    ...
    with tracker.start_run(run_name=model_name):
        ...
        tracker.log_params({
            ...
            "device": device,
            "optimizer" : optimizer,
            "seq_len": hipers.seq_len if m_type != "mlp" else None,
