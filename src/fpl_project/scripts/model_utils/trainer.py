...
class Trainer:
    ...
    def _adjusted_r2(self, r2, n_samples):
        ...
        return 1 - ((1 - r2) * (n_samples - 1) / (n_samples - self.num_features - 1))
    ...
    def test_step(self, dataloader, model, loss_fn):
        ...
        n_samples = len(dataloader.dataset) 
        return total_loss / len(dataloader), *self._compute_metrics(n_samples), torch.cat(preds_all)
    ...
    def model_eval(self, train_dataloader, test_dataloader, model, optimizer, loss_fn,
        ...
        for epoch in tqdm(range(num_epochs), desc=model.__class__.__name__):
            ...
            (tr_loss,
             ...
             tr_adj_r2) = self.train_step(train_dataloader, model, optimizer, loss_fn)

            (ts_loss,
             ts_mse_b, ts_mse_m, ts_mse_p,
             ts_mae_b, ts_mae_m, ts_mae_p,
            ts_adj_r2, _) = self.test_step(test_dataloader, model, loss_fn)
